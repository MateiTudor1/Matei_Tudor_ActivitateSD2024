...
void afisareVectorCalculator(struct Calculator* vector, int nrCalculatoare)
{
...
struct Calculator* copiazaCalculatatoareCuMultePorturi(struct Calculator* calculatoare, int nrCalculoatoare, int pragNrPorturi, int* nrCalculatoarePorturi)
{
	*nrCalculatoarePorturi = 0;
	for (int i = 0; i < nrCalculoatoare; i++)
	{
...
void main()
{
	...
	afisareVectorCalculator(calculatoare, nrCalculatoare);
	...
	for (int i = 0; i < nrCalculatoareCopiate; i++)
	{
		afisareCalculator(calculatoareCopiate[i]);
	}

	int nrCalculatoarePorturi = 0;
	struct Calculator* CalculatorcalculatoarePorturi = copiazaCalculatatoareCuMultePorturi(calculatoare, nrCalculatoare, 3, &nrCalculatoarePorturi);

	printf("\n\n\n");

	afisareVectorCalculator(CalculatorcalculatoarePorturi, nrCalculatoarePorturi);
